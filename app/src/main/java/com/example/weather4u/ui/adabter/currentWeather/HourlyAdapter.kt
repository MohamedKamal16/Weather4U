...
class HourlyAdapter(private var hoursList: MutableList<HourlyItem?>) : RecyclerView.Adapter<HourlyAdapter.HourlyViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int)=
        HourlyViewHolder(HourlyRowBinding.inflate(LayoutInflater.from(parent.context), parent, false))
        ...
        with(holder.binding){
           ...
           tvCloudValueHr.text = (hoursList[position]?.clouds).toString()
            ...
            CoroutineScope(Dispatchers.Main).launch{
                Glide.with(imageView3.context).load(Icon.getIcon(hoursList[position]?.weather?.get(0)?.icon)).into(imageView3)
            }
        }
    }
